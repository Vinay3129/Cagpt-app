...
def list_and_download_files(client, bucket_name, temp_dir):
    """Lists and downloads all files from the R2 bucket, and returns the documents and subjects."""
    all_docs = []
    subjects = set()
    ...
    return all_docs, list(subjects)

def prepare_vectorstore():
    """
    Prepares the FAISS vector store and returns it along with the list of subjects.
    """
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        all_docs, subjects = list_and_download_files(r2_client, bucket_name, temp_dir)
